        ...
        <main className='main'>
            ...
            {noteData?.content?.length > 0 && noteData?.totalPages > 1 &&
                <div className='pagination'>
                    <a
                        onClick={() => getAllNotes(currentNotePage - 1)}
                        className={currentNotePage === 0 ? 'disabled' : ''}
                    ...
                    {noteData && [...Array(noteData.totalPages).keys()].map((page) =>
                        <a
                            onClick={() => getAllNotes(page)}
                            className={currentNotePage === page ? 'active' : ''}
                            key={page}
                    ...
                    <a
                        onClick={() => getAllNotes(currentNotePage + 1)}
                        className={noteData.totalPages === currentNotePage + 1 ? 'disabled' : ''}
